...
UI_BASE_TAG_PATTERN = re.compile(r'<base href="/ui/"\s*/?>')


def render_ui_index(index_html: str, root_path: str) -> str:
    """Point the UI's <base> at the prefix this server is served under."""
    replacement = f'<base href="{escape(root_path)}/ui/">'
    # A function replacement keeps a backslash in root_path literal.
    return UI_BASE_TAG_PATTERN.sub(lambda _: replacement, index_html)
...
def build_app(
    ...
    if SHARE_TILED_PATH:
        ...
        # Normalized by Config's validator before it reaches here.
        configured_root_path: str = server_settings.get("root_path", "")
        index_html_path = Path(SHARE_TILED_PATH, "ui", "index.html")
        index_html: Optional[str] = (
            index_html_path.read_text() if index_html_path.is_file() else None
        )
        if index_html is not None and not UI_BASE_TAG_PATTERN.search(index_html):
            logger.warning(
                f"{index_html_path} has no <base> tag to rewrite. The UI will "
                f"only work at the root of its domain."
            )
        ...
        async def lookup_file(request: Request, path, try_app=True):
            ...
            if path == "index.html" and index_html is not None:
                return Response(
                    content=render_ui_index(index_html, request_root_path(request)),
